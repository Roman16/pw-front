            ...
            {localRegions.map((region, index) =>
                <li className={`${openedRegions.includes(index) ? 'opened' : ''} ${activeRegion && activeRegion.id === region.id ? 'active' : ''}`}>
                    <div className="account-name" onClick={() => openRegionHandler(index)}>
                        ...
                        {(region.mws_access_status !== 'CREDENTIALS_SUCCESS' || region.amazon_ads_api_access_status !== 'CREDENTIALS_SUCCESS' || region.is_amazon_ads_api_attached === false || region.is_amazon_sp_api_attached === false) &&
                        <InformationTooltip
                            type={'custom'}
