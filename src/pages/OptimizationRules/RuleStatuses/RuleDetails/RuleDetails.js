...
export const RuleDetails = ({
    ...
    const getRuleData = async () => {
        ...
        try {
            const {result} = await optimizationRulesServices[activeTab === tabs[0] ? 'getRuleLogs' : 'getRuleStatuses']({
                ...requestParams,
    ...
    }, [selectedRule, requestParams, activeTab])

    return (<div className="rule-details">
        <div className="actions">
            <h2>{selectedRule?.name}</h2>

            {selectedRule.id ? selectedRule.active ? <button className="btn grey" onClick={activateRuleHandler}>
                    <SVG id={'pause'}/>
                    Pause
                </button>
                :
