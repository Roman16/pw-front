...
export const Accounts = ({accounts, selectedAccount, activeRegion, activeMarketplace, onSelect, onSetMarketplace}) => {
    ...
    return (
        <ul className="accounts-list">
            {accounts.map(({account_alias, seller_id, amazon_region_account_marketplaces, id, mws_access_status, amazon_ads_api_access_status, is_amazon_ads_api_attached, is_amazon_sp_api_attached}, index) => (
                <li className={selectedAccount?.id === id && 'active'}>
                    <div className="account-name" onClick={() => onSelect(accounts[index])}>
                        ...
                        {(mws_access_status !== 'CREDENTIALS_SUCCESS' || amazon_ads_api_access_status !== 'CREDENTIALS_SUCCESS' || is_amazon_ads_api_attached === false || is_amazon_sp_api_attached === false) &&
                    ...
                    <ul className={`marketplace-list ${openedAccount === index ? 'opened' : ''}`}>
                        {amazon_region_account_marketplaces.map(({marketplace_id, profile_id}, mIndex) => {
                            const countryName = marketplaceIdValues[marketplace_id]?.countryName || ''
                            ...
                            return (
                                <li>
                                    ...
                                    {activeRegion.id === id && activeMarketplace.marketplace_id === marketplace_id ?
